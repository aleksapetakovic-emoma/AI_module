...
# option_por = tk.Checkbutton(w, variable, *OPTIONS).grid(row=3, column = 1)
...
def evaluate():
    ...
    os.chdir(cmd)
    process = subprocess.call('\"C:/Program Files/swipl/bin/swipl\" -f prolog_code_ks.pl < query.txt > output.txt',
                              shell=True)
    f = open("output.txt", "r")
    risk = f.read()
    try:
        messagebox.showinfo("Risk Evaluation", "Expert System (prolog): "+risk+"\n"+"Tensorflow: " + str(rank) + "({:4.1f}%)".format(probability) +  "\n"+"Random Forests: " + prediction_rf )
    except:
        messagebox.showinfo("Risk Evaluation", "Expert System (prolog): "+risk)
...
#tk.Button(w,
#          text='Check Age',  command=check_age).grid(row=7, column= 1)
tk.Button(w,
